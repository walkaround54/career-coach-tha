...
rating_summary = df.groupby("rating_text")["user_aggregate_rating"].describe()
...
mean_ratings = df.groupby("rating_text")["user_aggregate_rating"].mean()
mean_ratings = mean_ratings.reindex(ordered_classes, fill_value=0)

ax3.scatter(mean_ratings.index, mean_ratings.values, color="red", alpha=0.7)
...
box_data = df.groupby("rating_text")["user_aggregate_rating"].describe()
